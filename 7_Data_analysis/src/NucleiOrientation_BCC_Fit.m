function [uFCCind,vec,blenFit,ori]=NucleiOrientation_BCC_Fit(indmodel,uCorner,blen,del)
plt=0;
% del=0.15;
...
[para0, ~,~] = lsqcurvefit(@BCClatticeErr, x0, xdata, zeros(3,length(xdata.uCorner)), lb, ub, opt);
...
ref=repmat(orif,[1 length(xdata.uCorner)])+(vecf1*xx+vecf2*yy+vecf3*zz)*blenf;
